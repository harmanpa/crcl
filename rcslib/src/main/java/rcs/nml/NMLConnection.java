 ...
 * <A HREF="mailto:shackle@cme.nist.gov">shackle@cme.nist.gov</a>
 *
 */
public class NMLConnection implements NMLConnectionInterface {
    ...
    /**
     * Get the time in milliseconds when the last connect() was attempted. (To
     * determin how long it has been since the last connection subtract from the
     * current time given with System.currentTimeMillis();
     *
     * @return connect_time
     */
     ...
     * (To determin how long it has been since disconnected subtract from the
     * current time given with System.currentTimeMillis();
     *
...
//        return s;
     ...
     * the message is new. A message in new if its id does not equal the value
     * set here.
     *
    ...
    /**
     * When the NMLConnection performs a verify bufname (which it normally does
     * in the constructor) it requests the buffername from the server that
     * corresponds to the selected port and buffer number. If this is not the
     * same as the buffer passed to the constructor it indicates that
     * configuration used by the server is inconsistant with the one being used
     * by this process. Regardless the value is stored and can be retrieved
     * later with get_returned_buffer_name().
     *
...
//            }
    ...
    /**
     * Set config_debug_on to true to print out additional information while
     * reading the NML Congiguration file.
     */
    static private boolean config_debug_on = rcs.nml.debugInfo.debug_on;
    /**
     * Set read_debug_on to true to print out additional information while
     * reading.
     */
    private boolean read_debug_on = rcs.nml.debugInfo.debug_on;
    /**
     * Set write_debug_on to true to print out additional information while
     * writing.
     */
    ...
    /**
     * True when this object is connected to the NML server. All reads and
     * writes will fail when this is false.
     */
...
//    }
    ...
    /**
     * Add an opject to be called with each error to append messages. Multiple
     * appenders can be added.
     *
...
//            StackTraceElement ste[] = t.getStackTrace();
    ...
    /**
     * This constructs an NMLConnection which will not work until the host and
     * port are set manually or by reading the NML configuration file with
     * ReadNMLConfigurationFile(). Most users should use the other constructor
     * with several parameters.
     *
     * @see rcs.nml.NMLConnection#NMLConnection(rcs.nml.NMLMessageDictionary,
     * java.lang.String, java.lang.String, java.lang.String)
     * @see rcs.nml.NMLConnection#ReadNMLConfigurationFile(java.lang.String,
    ...
    /**
     * This functions sets the NMLMessageDictionary for this connection. The
     * purpose of the NMLMessageDictionary is to provide a means for the
     * NMLConnection to determine the structure of messages being sent or
     * received using a message type. (An integer passed within all NML
     * messages.)
     *
    ...
    /**
     * Set an interface that will have its member functions called for various
     * errors. Used by the diagnostics tools to improve error reporting.
     *
     ...
     *
     * @see
     * rcs.nml.NMLConnection#SetMessageDictionary(rcs.nml.NMLMessageDictionary)
     * @see rcs.nml.NMLMessageDictionary
     */
    ...
    /**
     * This functions sets the NMLFormatConverter for this connection. The
     * NMLFormatConverter is resposible for converting each of the basic data
     * types to some neutral format that can be used on many different
     * platforms. Most users should either accept the default
     * (XDRFormatConverter) or allow the format to be specified in the NML
     * configuration file rather than calling this function directly.
     *
     * @param new_fc the NMLFormatConverter for this Connection to use.
     * @throws rcs.nml.NMLException when nml_fc is not extended from
    ...
    /**
     * This constuctor creates a fully functional NML Connection using the
     * application defined NMLMessageDictionary, read the NML configuration
     * file, and connects immediately. This is the constructor most users should
     * use.
     *
     * @param msg_dict the NMLMessageDictionary for this connection to use (the
     * NMLMessageDictionary allows this Connection to determine the message
     * structure from the message type)
     * @param BufferName the name of the buffer to connect to, (must match one
     * of the buffers in the configuration file.)
     * @param ProcessName the name of the process that will use this connection
     * (must match one of the process names in the configuration file)
     * @param ConfigurationFile the file name or URL of an NML configuration
     * file (URL's should either be complete, or they can be relative to
     * rcs.utils.URL_and_FileLoader.current_directory)
     * @exception rcs.nml.NMLException If the configuration file can not be
     * read, the file is improperly formatted or the buffer or process can can
     * not be found in it.
     *
     */
    ...
    /**
     * This function reads configuration information from the NML configuration
     * file.
     *
     * @param BufferName the name of the buffer to connect to, (must match one
     * of the buffers in the configuration file.)
    ...
    /**
     * Parse or return configution info from a previously parsed NML
     * configuration file.
     *
    ...
    /**
     * Should NML store configuration data to make connecting to new buffers
     * faster?
     *
     * Default : true
     */
    ...
    /**
     * Connect or reconnect to the sockets as appropriate. Most errors throw an
     * exception. Unfortunately the return value also needs to be checked since
     * a few errors only cause a -1 return value.
     *
     * NOTE: ReadNMLConfigurationFile and the NMLConnection() constructor with 4
     * arguments. call connect() internally so it is only necessary to call this
     * if the first connect() failed, or disconnect() was called or the 0
     * arguments constructor was used to createt the NMLConnection. * @return 0
     * connect was ok. -1 error occured.
     *
...
//					m_InputStream = new DataInputStream(m_Socket.getInputStream());
    ...
    /**
     * Disconnect this object from the NML server. This allows the NML server to
     * shutdown and come back up while preserving the configuration information
     * in this object.
     *
...
//                }
     ...
     * by this process.
     *
     * @return true if the host could be contacted and returned the expected
     * buffername, or false if the server could not be contacted or the
     * buffernames do not match.
     */
     ...
     * Read a NMLmsg.
     *
     * @return null if the message in the buffer has already been read by this
     * NMLConnection, otherwise it returns the NMLmsg read.
     *
     * @exception rcs.nml.NMLException The read failed (ussually because of some
     * network error).
     *
     */
    ...
    /**
     * Send additional diagnostics info to server so it can better track which
    ...
    /**
     * Setup a single variable log. Occationally it is useful to read only a
     * single variable from a large message and there for reduce the amount of
     * bandwidth required to log that variable.
     *
     * @param varname name of the variable to log
     * @param maxlogsize maximum number of variable updates to store
     * @param period time in seconds between checks to see if the variable
     * changed.
     * @param type type of expected message, no data is logged when other
     * message types are in the buffer.
    ...
    /**
     * Get a NMLSingleVarLog object associated with the var_log_number that
     * should have been returned by setupSingleVarLog()
     *
    ...
    /**
     * Tell the server to send this process messages from the buffer every
     * period seconds without needing a request message.
     *
...
//            }
     ...
     *
     * @return null if the message in the buffer has already been read by this
     * NMLConnection or no message has yet been written to the buffer, otherwise
     * it returns the NMLmsg read.
     *
     * @exception rcs.nml.NMLException The peek failed.
     */
...
//			throw create_NMLException("input buffer not ready.");
     ...
     * @return null if the message in the buffer has already been read by this
     * NMLConnection or no message has yet been written to the buffer, otherwise
     * it returns the NMLmsg read converted to a string. The string is a comma
     * separated list of the parameters in the NMLmsg in the order they are
     * updated, starting with the type and size.
     *
     * @exception rcs.nml.NMLException The read failed.
     *
    ...
    /**
     * Convert the string into an NMLmsg and send it to this channel. Depending
     * on the server configuration the string may also be sent directly to the
     * server to be converted on the server end. The string should be comma
     * delimited starting with the type and size. ie: 1001,0,99
     *
     ...
     * delimited starting with the type and size. ie: 1001,0,99
     *
     * The same as writeDataString except it never throws an exception merely
    ...
    /**
     * Get the host name where the buffer is supposed to be located. This is
     * normally obtained from the BufferLine of the config file or from the
     * nmlcfgsvr.
     *
     * @return host
     */
    ...
    /**
     * Set the host name where the buffer is supposed to be located which will
     * be used for the next connection attempt. Either host names or IP
     * addresses can be used. This is normally obtained from the BufferLine of
     * the config file or from the nmlcfgsvr.
     *
    ...
    /**
     * Set the name of the NML configuration file. This should be followed with
     * ReadNMLConfigurationFileNoThrow() to actually have the configration file
     * read.
     *
     ...
     * Get the buffer name.
     *
     * This is normally set by being passed as an argument to the
     * NMLConnection() constructor.
     *
     * @return buffer_name
     */
    ...
    /**
     * Read a message and convert it to a comma delimeted string. Depending on
     * the server configuration the string might also be sent from a server that
     * did the conversion on the server side.
     *
     * It marks the buffer as read and removes the message from the queue if the
     * buffer is queued.
     *
    ...
    /**
     * Peek a message and convert it to a comma delimeted string. Depending on
     * the server configuration the string might also be sent from a server that
     * did the conversion on the server side.
     *
     * Peeking is the same as reading except that it does not modify the buffer
     * it reads. It does not mark the buffer as read or remove the message from
     * a queued_buffer.
     *
    ...
    /**
     * Return the TCP or UDP port number associated with this connection. This
     * is normally obtained from the BufferLine of the config file or from the
     * nmlcfgsvr.
     *
     * @return port number
     */
    ...
    /**
     * Set the TCP or UDP port to be used for the next time a connect() is
     * performed. This is normally obtained from the BufferLine of the config
     * file or from the nmlcfgsvr.
     *
    ...
    /**
     * Get the buffer_number for this buffer. The buffer number is sent with
     * each request to the server to identify which buffer should be used. This
     * is normally obtained from the BufferLine of the config file or from the
     * nmlcfgsvr.
     *
    ...
    /**
     * Set the buffer_number for this buffer. The buffer number is sent with
     * each request to the server to identify which buffer should be used. This
     * is normally obtained from the BufferLine of the config file or from the
     * nmlcfgsvr.
     *
     ...
     *
     * Same as ReadNMLConfiguration() except errors are only printed to stderr
     ...
     *
     * ReadNMLConfigurationFile and the NMLConnection() constructor with 4
     * arguments. call connect() internally so it is only necessary to call this
     * if the first connect() failed, or disconnect() was called or the 0
     * arguments constructor was used to createt the NMLConnection.
     *
    ...
    /**
     * Get the value of the connected property. Set to true by a successful
     * connect() or constructor set to false by disconnect().
     *
