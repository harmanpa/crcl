...
/**
 * This class provides the same interface as java.io.PipedOutputStream except
 * that it corrects the problem that when java.io.PipedOutputStream has 1k or
 * more written to it, it blocks until the some of the data is read from the
 * input pipe before more can be written.
 * CorrectedPipedInputStream/CorrectedPipedOutputStream only block for mutual
 * exclusion but will allow any amount of data(atleast until you run out of
 * memory) to be written to the pipe without waiting for a read.
 *
 * <pre>
